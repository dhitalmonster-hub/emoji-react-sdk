...
describe('isValidContractName', () => {
  it('accepts standard names', () => {
    expect(isValidContractName('emoji-react')).toBe(true);
...
describe('parseContractId / formatContractId', () => {
  it('parses valid id', () => {
    const got = parseContractId('SP20Z3WPE6PVN1B8APDQNDH5BR1AJNMR25QPCSGFT.emoji-react');
    expect(got?.name).toBe('emoji-react');
  });
